...
| V2: original components | The real Garmin chart, its pointer interaction, local images and nested layout render in place. A nested Footnote edit preserves surrounding source and shares body undo/redo. A controlled fixture initializes the real Garmin chart while its original DOM is staged out of view, then checks that reattachment restores its dimensions. | Focused checks pass, including the controlled reattachment regression. |
