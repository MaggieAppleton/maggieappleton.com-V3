...
Independent review added regression coverage for inherited tab identities and protected-source recovery after offset changes. Both original browser cases failed before their fixes. The four focused recovery regressions and all 117 editor unit tests pass; a repeated-region source test covers nested writing components. Recovery-choice cases cover archival before replacement, storage failure, newer engine snapshots after conversion failure, and superseded save retries. Review and repro evidence is under `independent-sol-review/` and `recovery-fixes/`.
