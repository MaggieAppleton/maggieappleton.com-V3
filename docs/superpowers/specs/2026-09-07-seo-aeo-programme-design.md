...
Replace the generic landing-page description with collection-aware copy while
leaving authored content frontmatter unchanged. Missing description policy for
Now and Smidgeons will be encoded explicitly rather than bulk-filled into
individual content entries.

Acceptance:

- Every indexable landing page has a concise, unique description.
- Existing article-like content continues to use its authored frontmatter
  description without copy edits. Now and Smidgeon templates may use a
  collection-specific fallback only when the page is not represented as an
  Article and the fallback is asserted in focused tests.
- No enabled page-metadata description is empty, `...`, duplicated from the site
  default, or generated by truncating arbitrary body text.
