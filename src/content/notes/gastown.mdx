...
And yet it's already lit divisive fires and sparks of change across the software engineering community. First because it's utterly unhinged, and second because it's a serious indication of how agents will change the nature of software development from this point on.
...
We should take Yegge's creation seriously not because it's a serious, working tool for developers (it isn't). But because it's a good piece of speculative design fiction that asks provocative questions and reveals the shape of constraints we'll face as agentic coding systems mature and grow.
...
Yegge didn't invent this pattern of tracking atomic tasks outside agent memory in something structured like JSON. Anthropic described the same approach in their research on [effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents), just published in November 2025. I give it a hot minute before this lands in Claude Code.

### Feeding agents continous streams of work

The whole promise of an orchestration system like Gas Town is it's a perpetual motion machine. You simply give high level orders to the mayor, and a zoo of agents runs off to break it down into tasks, assign them, execute them, check for bugs, fix the bugs, review the code, and merge it in.

Keeping the system running 

Every agent in Gas Town has a "hook" that points to its current work queue. When a task finishes, new work gets added. The goal here is agents are never idle: there's always something waiting for them.
