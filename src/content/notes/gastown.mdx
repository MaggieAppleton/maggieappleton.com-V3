...
This hierarchical approach solves both a coordination and attention problem. Without it, you the human are the one assigning tasks to dozens individual agents, checking who's stuck, who's idle, and who's waiting on work from someone else. With the Mayor as your single interface, that overhead disappears. The Mayor in Gas Town never writes code. [It assigns work to polecats, ...?]

Here's the basic relationship structure of Gas Town, as best I can make out:
...
Although I'll give Nano Banana and Steve points for world building and distintive characters. But I think still there's a lot of opportunity to diversify the cast. The agents in Gas Town are all generalist workers in the software developement pipeline. But we could add in any kind of specialist we want. dev ops, product managers, front-end debuggers. Think of Claude's Skills but applied to specialist custom agents.
...
This solves one of the major limitations of current agentic workflows: running out of context. Even before you hit the literal limits of a context window, [context rot]() will degrade the output enough that you constantly need to compact or start fresh sessions. Gas Town's solution is to liberally let sessions die - when one crashes or fills up, a fresh session spins up and inherits the same persistent identity and work queue from Git.
