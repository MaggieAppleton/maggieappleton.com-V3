...
import WorkQueues from "../../components/unique/gastown/WorkQueues.astro";
...
<IntroParagraph>A few weeks ago Steve Yegge published an elaborate [manifesto and guide](https://steve-yegge.medium.com/welcome-to-gas-town-4f25ee16dd04) to Gas Town, his Mad-Max-Slow-Horses-Waterworld-etc-themed agent orchestrator that runs dozens of coding agents simultaneously in a metaphorical town of automated activity. Gas Town is entirely vibecoded, hastily designed with off-the-cuff solutions, and inefficiently burning through thousands of dollars a month in API costs.</IntroParagraph>

This doesn't sound promising, but it's lit divisive debates and sparks of change across the software engineering community. A small hype machine has formed around it. It's made the rounds through every engineering team's Slack, probably twice.  There's somehow already a [$GAS](https://bags.fm/7pskt3A1Zsjhngazam7vHWjWHnfgiRump916Xj7ABAGS) [meme coin](https://en.wikipedia.org/wiki/Meme_coin) doing almost $400k in earnings.<Footnote idName={1}>This was not Yegge's doing – someone else set it up on the crypto platform [Bags](https://bags.fm/) which ties tokens to individual creators. The coin holds no legitimate relationship to Gas Town's success or failure so this is the purest of pure speculative betting. I expect nothing less from the crypto bros.</Footnote> And the hype is justified. First, because it's utterly unhinged, and second because it's a serious indication of how agents will change the nature of software development from this point on.
...
<Subtext>“Orange and Yellow" and "Royal Red and Blue" by Mark Rothko<br />Paintings I could totally make myself, but did not</Subtext>
...
This saving and recalling is all done through Gas Town's "Beads" system.<Footnote idName={3}>Beads is also the name of the [memory management system](https://github.com/steveyegge/beads) Yegge built before Gas Town; a precursor to this more ambitious civilisation.</Footnote> Beads are tiny, trackable units of work – like issues in an issue tracker – stored as JSON in Git alongside your code. Each bead has an ID, description, status, and assignee. Agent identities are also stored as beads, giving each worker a persistent address that survives session crashes.

Beads chain together into "molecules" – linked sequences of tasks that form a workflow. A release workflow might include bumping the version number, running linters, waiting for CI, building, and publishing. Each step has its own bead with acceptance criteria. If an agent crashes mid-molecule, the next session picks up the chain, sees which steps are done, and carries on from there.
...
Each worker agent in Gas Town has its own queue of assigned work and a "hook" pointing to the current thing they should be doing. The minute they finish a task, the next one jumps to the front of the queue. The mayor is the one filling up these queues – it's in charge of breaking down large features into atomic tasks and assigning them to available workers. In theory, the workers are never idle.

<WorkQueues />

But this turns out to be surprisingly difficult to make happen. Current models are trained to be helpful assistants who wait politely for human instructions. They're not trained to check a task queue and independently get on with their work. Gas Town's solution is aggressive prompting and constant nudging. Supervisor agents spend their time poking workers to see if anyone's stalled out or run dry on work. When one goes quiet, they send it a ping which jolts the agent into checking its queue and getting back to work. These periodic nudges move through the agent hierarchy like a heartbeat keeping everything moving.

### Merge queues and agent-managed conflicts

When you have a bunch of agents all working in parallel, you're of course going to run into merge conflicts. Each agent is off on its own branch, and by the time it finishes its task, the main branch might look completely different – other changes have landed, the code has moved on. The later an agent finishes, the worse this gets. Normally you'd have a human there to sort out the mess and decide which changes to keep. But if agents are running on their own, something has to do that job for them.

Gas Town has a dedicated merge agent – the Refinery – that works through the queue one change at a time. It looks at each merge request, figures out the conflicts, and gets it into main. When things get really tangled – when so much has changed that the original work doesn't even make sense anymore – it can creatively "reimagine" the changes: re-doing the work to fit the new codebase. Or escalate to a human if needed.

But there's another way to help sidestep merge conflict nightmares: stacked diffs.
[BORING: The traditional workflow has each feature sitting on its own branch for days or weeks, accumulating commits, then getting merged back as one chunky PR. Stacked diffs flip this: you break work into small, atomic changes that each get reviewed and merged on their own, building on top of each other. This fits how agents naturally work. They're already producing discrete, focused changes rather than sprawling multi-day branches. Smaller changes mean smaller merge surfaces, and when conflicts do pop up, they're easier to untangle because each diff touches less code. Cursor's acquisition of Graphite, a tool built specifically for stacked diff workflows, suggests agentic coding companies see this as the way forward. When you've got dozens of agents landing changes continuously, you need infrastructure designed for high-frequency, incremental merges, not the occasional big one.]
...
I can't find any mention online of the per-account limits, but let's conservatively assume he's spending at least $2,000 USD per month, and liberally $5,000.<Footnote idName={5}>Now covered by the [$75,000 he's earned](https://steve-yegge.medium.com/bags-and-the-creator-economy-249b924a621a) in transaction fees from the $GAS cryptocoin, which gives him a good year to run this at scale</Footnote>
...
Is it worth opening a full IDE when you can just ask the agent to make that change for you?
...
This all changes over the next 12-24 months. As models improve, trust will increase and tolerance for distance will grow. Backend and infrastructure work may shift dramatically toward distance. But some domains—UI/UX, creative coding, performance optimization—may remain proximity-dependent. New tools might bridge the gap: better visualization, semantic diffs, AI-assisted code review.
...
Gas Town has sent ripples across the lake. I'm already seeing spin offs and explorations that head towards far more useable and sensible versions.

The most valuable tools in this new world will facilitate critical thinking, planning, coordination.